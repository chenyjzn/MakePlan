...
class GanttChartGroup : View {
    ...
    private val taskHeight = 80.toPx()
    ...
    private val fontTaskOffsetY = -(ganttTextPaint.fontMetrics.top + ganttTextPaint.fontMetrics.bottom) / 2
    private val fontTimeLineOffsetY = -(timeLine2TextPaint.fontMetrics.top + timeLine2TextPaint.fontMetrics.bottom) / 2
    private val fontToolBarOffsetY = -(toolBarTextPaint.fontMetrics.top + toolBarTextPaint.fontMetrics.bottom) / 2
    ...
    private fun setProjectTimeByDx(dx: Float, width: Int): Pair<Long, Long> {
        val timeOffset = ((endDate - startDate).toFloat() * dx / width.toFloat()).toLong()
        return startDate - timeOffset to endDate - timeOffset
    }

    private fun checkProjectBound() {
    ...
    private fun setProjectTimeByDlDr(dl: Float, dr: Float, width: Int): Pair<Long, Long> {
        val timeOffsetL = ((endDate - startDate).toFloat() * dl / width.toFloat()).toLong()
        val timeOffsetR = ((endDate - startDate).toFloat() * dr / width.toFloat()).toLong()
    ...
    private fun calScale(Scale: Long): Float {
        return ((Scale.toFloat() / (endDate - startDate).toFloat()) * width.toFloat()).toDp()
    }

    private fun setTimeLineScale() {
    ...
    private fun interpolation(startTime: Long, endTime: Long, actualTime: Long): Float {
    ...
    private fun drawPrimaryTimeLineByDay(canvas: Canvas) {
        ...
        while (actualTime <= extendEndTime) {
            ...
            canvas.drawText(text, textXPos, timeLineHeight.toFloat() * 0.25f + fontTimeLineOffsetY, timeLine1TextPaint)
            prePos = curPos
        }
    }
    ...
    private fun checkTaskModeTouchPos(x: Float, y: Float): TouchMode {
        taskList?.let {
            for ((index, value) in it.withIndex()) {
                val left = interpolation(startDate, endDate, value.startTimeMillis) * width.toFloat()
                val right = interpolation(startDate, endDate, value.endTimeMillis) * width.toFloat()
    ...
    private fun getTaskSelect(x: Float, y: Float): Pair<Int, Task?> {
        taskList?.let {
            for ((index, value) in it.withIndex()) {
                val left = interpolation(startDate, endDate, value.startTimeMillis) * width.toFloat()
                val right = interpolation(startDate, endDate, value.endTimeMillis) * width.toFloat()
    ...
    private fun getTaskLongSelect(y: Float): Int {
        taskList?.let {
            for ((index, value) in it.withIndex()) {
    ...
    private fun checkTaskSwapByYPos(yPos: Float) {
        taskList?.let { list ->
    ...
    private fun setTaskTimeOffsetByDx(dx: Float, width: Int): Long {
                ...
                MotionEvent.ACTION_MOVE -> {
                    if (event.pointerCount == 1) {
                        ...
                        if (touchStatus == TouchMode.ZOOM) {
                            ...
                            val dl = hypot(newXL - centerX, nerYL - centerY) - hypot((oldXL - centerX), (oldYL - centerY))
